# pong_client.py (Phiên bản cuối cùng đã sửa lỗi)
import pygame
import socket
import pickle
import threading
...
def draw_game(screen, state):
    ...
    name1_text = font_small.render(state["player_names"][0], True, WHITE)
    score1_text = font_medium.render(str(state["scores"][0]), True, WHITE)
    name2_text = font_small.render(state["player_names"][1], True, WHITE)
    ...
    if state["game_over"]:
        ...
        winner_text = font_large.render(f"{winner_name} Wins!", True, WHITE)
...
def draw_menu(screen, input_text, difficulty, player_id, status_text=""):
    ...
    title_text = font_large.render("Multiplayer Pong", True, WHITE)
    title_rect = title_text.get_rect(center=(WIDTH // 2, HEIGHT // 4))
    screen.blit(title_text, title_rect)
    
    if player_id == 0:
        instruction_text = font_small.render("Enter your name & choose difficulty. Press ENTER to start.", True, WHITE)
        difficulty_text = font_small.render(f"Difficulty: {['Easy', 'Medium', 'Hard'][difficulty]}", True, WHITE)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_RETURN:
                    if my_id == 0:
                        data_to_send = {"name": input_text if input_text else "Player 1", "difficulty": difficulty}
                    else:
                        data_to_send = input_text if input_text else "Player 2"
